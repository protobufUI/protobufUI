...
import ipetoolkit.workspace.WorkspacePersistence.Loaded
...
object CliMain {
  def start(config:Config)={
    ...
    val loadedMsg = Await.result((workspacePersistence ? LoadWorkspace(config.workspace.getAbsolutePath)).mapTo[Loaded], 30 seconds)
    loadedMsg match {
  ...
  def runTests(config: Config, rootEntry: RootEntry) = {
    ...
    Globals.setProperty(Globals.Keys.workspaceRoot,config.workspace.getAbsolutePath)
