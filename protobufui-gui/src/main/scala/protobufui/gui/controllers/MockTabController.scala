...
import akka.actor._
import com.google.protobuf.{TextFormat, MessageLite, UnknownFieldSet}
...
class MockTabController extends Initializable with InvalidationListener with DetailsController {
  ...
  def createMockDefinition = {
    ...
    MockDefinition(new InetSocketAddress(port), new MessageLiteClass(classOf[UnknownFieldSet]), responseGen)
  }
